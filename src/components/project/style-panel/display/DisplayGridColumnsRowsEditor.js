...
export default function DisplayGridColumnsRowsEditor(props) {
  const displayStyle = useSelector(
    (state) => state.project.activeStyleObject?.['grid-template-columns']
  )
  ...
  function handleDeleteColumn(columnId) {
    let deletedColumnIndex = columns
      .map((x) => {
        return x.id
      })
      .indexOf(columnId)
    ...
    setColumns((columns) =>
      columns.filter((column, i) => i !== deletedColumnIndex)
    )
  }
    ...
    if (columns.length > 0) {
      dispatch(
        editStyleProperty([
          ...
          columns.map((column) => column.size).join(' '),
        ])
      )
    }
  }, [columns])
  ...
  return (
    <div className="display-horizontal-grid with-margin">
      ...
      <ProprtyInputLabel text="Columns" property="grid-template-columns" />
      <div style={{ width: '100%' }}>
        <div style={{ textAlign: 'right', width: '100%', marginBottom: '4px' }}>
          <div className="plusButton" onClick={handleAddColumn}>
            +
          </div>
        </div>
        ...
        {columns.map((column) => (
          <div className="display-grid-column" key={column.id}>
            <DisplayGridColumnSizeButton
              id={column.id}
              size={column.size}
              handleEditColumnSize={handleEditColumnSize}
              handleEditColumnUnitChange={handleEditColumnUnitChange}
            />
            <div
