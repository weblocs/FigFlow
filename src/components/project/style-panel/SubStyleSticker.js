...
export default function SubStyleSticker({
  ...
  function handleDeleteStyleSubOption(subOptionId) {
    dispatch(
      deleteStyleSubOption({ optionIndex: index, subOptionId: subOptionId })
    )
  }
  ...
  return (
    <div
      ...
      className={
        'selected-class' +
        (activeStyleId === id ? ' active' : '') +
        (styleIsSet ? '' : ' styleIsNotSet')
      }
      style={{ zIndex: stylesInActiveNode.length + 10 - index }}
    >
              ...
              ?.childrens[index].options.map((option) => (
                <div className="style-option-item" key={option.id}>
                  <div
                    ...
                    onClick={() =>
                      handleStyleOptionClick(option.id, option.name)
                    }
                  >
                    {option.name}
                  </div>
                  <div
                    className={
                      'style-option-item_icons' +
                      (editingOptionsTurnOn ? ' active' : '')
                    }
                  >
                    <div
                      className="style-option-item_delete-icon"
