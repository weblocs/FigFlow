...
import { setActiveSymbolConnections, setKeyboardNavigationOn } from "../../../../features/project";
import EditImg from '../../../../img/edit.svg';
import ConfirmDeleteModalButton from "../../modals/ConfirmDeleteModalButton"

export default function SymbolEditButton({element, editFx, deleteFx, active, text, itemType, folderItem}) {
    
    const activeSymbolConnections = useSelector((state) => state.project.activeSymbolConnections)
    const isConnectionsListEmpty = useSelector((state) => state.project.activeSymbolConnections.length === 0)
    ...
    useEffect(() => {
        if(isEditorOpened) {
            dispatch(setActiveSymbolConnections({id: element.id}));
        }
    ...
    return (
        <div>
            ...
            className={"edit-block_toolbox" + (isEditorOpened ? " active" : "")}>
                ...
                {!isConnectionsListEmpty &&
                <div className="edit-block_toolbox-text">Symbol is used:</div>
                }
                <div className="symbol-connections_list">
                {activeSymbolConnections.map((connection) => (
                    <div className="edit-block_toolbox-text" key={connection.nodeId}>
                        {connection.pageName} {connection.amount} time{connection.amount > 0 && "s"}
                    </div>
                ))}
                </div>
