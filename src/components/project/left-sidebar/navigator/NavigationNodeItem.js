...
function NavigationNodeItem({ parents, node, depth }) {
  ...
  const draggedNavigatorNodes = useSelector(
    (state) => state.project.draggedNavigatorNodes
  )
  const draggedOverNodeId = useSelector(
    (state) => state.project.draggedOverNodeId
  )
  const editedSymbolId = useSelector((state) => state.project.editedSymbolId)
  const navigatorItemDragBehindState = useSelector((state) =>
    state.project.navigatorItemDragBehindState ? true : false
  )
  ...
  function scrollProjectTo(id) {
    const actualNodePosition = document
      .querySelector(`[el_id="${id}"]`)
      ?.getBoundingClientRect().top
    if (
      actualNodePosition < 0 ||
      actualNodePosition > window.screen.height - 200
    ) {
      const actualViewportPosition =
        document.getElementById('project-view').scrollTop
    ...
    >
      <div
        onDoubleClick={() =>
          ...
          dispatch(
            makeSymbolEditable({ symbolId: node.symbolId, elementId: node.id })
          )
        }
        className={
          'navigation-node-inside' +
          (node.type === 'sym' ? ' symbol-nav-item' : '') +
          (depth >= 0 ? ' lined' : '') +
          (depth === 0 ? ' white' : '')
        }
      >
        {node.children.length > 0 && (
          <img
            src={Arrow}
            className={
              'node-folder-arrow' + (node?.expanded ? ' expanded' : '')
            }
