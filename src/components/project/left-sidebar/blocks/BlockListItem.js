...
export default function BlockListItem({
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      onClick={() =>
        dispatch(setActiveBlock({ id: element.id, folderId: folderId }))
      }
      richid={element.id}
      className={
        'projectPageItem edit-icon_wrapper ' +
        (activeBlockId === element.id ? 'active' : '') +
        (draggedOverIndex === index ? 'draggedOver' : '') +
        (draggedOverIndex === index + 1 && index === blocks.length - 1
          ? 'draggedOverBottom'
          : '')
      }
      key={element.id}
