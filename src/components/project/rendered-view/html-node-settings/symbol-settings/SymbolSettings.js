...
export default function SymbolSettings() {
    ...
    const activeSymbolName = useSelector((state) => {
        ...
        if(activeSymbolId !== undefined) {
           return state.project.projectSymbols.find(({id}) => id === activeSymbolId).name;
        } 
    })
    ...
    return (
        <>
            <div className={"rich-element-settings_button button-centered text-button" + 
            (isNodeSymbol ? " active" : "")}>
                Symbol: {activeSymbolName}
            </div>
            <div className="rich-element-settings_button button-centered active"
            onClick={() => (editedSymbolId.symbolId === "") && 
            dispatch(makeSymbolEditable({symbolId:activeNodeObject.symbolId, elementId: activeNodeObject.id}))}
            >✎</div>
        </>
    )
    }
